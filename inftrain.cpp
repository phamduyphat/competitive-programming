...
/*
    TH M == 1:
    khách (ai, +, id) (bi, -1, id)      a,b : vị trí; + && - : tính chất; sắp xếp các sự kiện theo thời gian(- -> +);
    T: số khách đang có trên tàu, ban đầu T = 0;
    Xét lần lượt các sự kiện:
        + t, -, id:     nếu id ko được phục vụ -> bỏ qua
                        nếu id được phục vụ -> tăng t lên(t ++), loại id khỏi danh sách người trên tàu;
        + t, +, id:     nếu tàu chưa đầy -> t ++, thêm id vào danh sách người trên tàu;
                        nếu tàu đầy rồi -> đuổi 1 khách trên tàu mà xuống muộn nhất > b(id);


    thay vì xét theo chiều tàu đi từ 0 đi thì đi từ 10^9 đi về;
    TH M > 1: xét chiều 1e9 -> 0 :  khách i:(b(i), +, i) (a(i), -, i)
                                    tàu j: (S, 2, id, -> 9)        sự kiện 2: nới rộng kích thước tàu lên(xét trước)
    -> cài tương đồng với M == 1;
*/
