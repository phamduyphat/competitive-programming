...
driver main(){
    ...
    sort(a.begin(), a.end(), [&](const input &x, const input &y)
    {
        return x.x > y.x;
    });
    pos = a[0].y;
    ...
    for(int64_t i = 0; i < a.size(); i ++)
    {
        ...
        {
            ans += pos - mul(k, mul(count, count - 1)) + pos;
            left_over = number - count * k;
            pos = a[i + 1].y;
            if (left_over)
            {
                ans += (a[i].y - count * k) * 2;
                left_over = k - left_over;
            }
        }
    }
    cout << ans << endline;
