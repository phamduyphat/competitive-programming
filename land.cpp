...
const int16_t maxN = 1e3 + 5;
int64_t a[maxN][maxN], s[maxN][maxN];
/***********************************************************/
//Functions declarations:
#define MID (l + (r - l) / 2)
void sub1(int32_t &n, int32_t &m, int32_t &k, int32_t &t)
{
    int64_t ans = 0;
    fr1(i, m)
        fr1(j, n)
        {
            int32_t l = 1, r = min(i, j);
            while (l <= r)
            {
                int32_t mid = MID;
                int64_t sum = s[i][j] - s[i - mid][j] - s[i][j - mid] + s[i - mid][j - mid] ;
                if (t >= sum)
                {
                    ans = max(ans, (int64_t) (mid));
                    l = mid + 1;
...
bool overlap(int32_t ii, int32_t jj, int32_t i, int32_t j, int32_t oo, int32_t kk, int32_t o, int32_t k)
{
    if (i >= oo && j >= kk && o >= ii && k >= jj)
        return false;
    return true;
}
void sub3(int32_t &n, int32_t &m, int32_t &k, int32_t &t)
{
    int64_t ans = 0;
    fr1(i, m)
        fr1(j, n)
            fr1(p, min(i, j))
            {
                fr1(o, m)
                    fr1(k, n)
                        fr1(q, min(o, k))
                        {
                            int32_t ii = i - p + 1, jj = j - p + 1;
                            int32_t oo = o - q + 1, kk = k - q + 1;
                            if (overlap(ii, jj, i, j, oo, kk, o, k))
                            {
                                int64_t s1 = s[i][j] - s[ii - 1][j] - s[i][jj - 1] + s[ii - 1][jj - 1];
                                int64_t s2 = s[o][k] - s[oo - 1][k] - s[o][kk - 1] + s[oo - 1][kk - 1];
                                if (s1 + s2 <= t)
                                ans = max(ans, p * p + q * q);
                            }
                        }
