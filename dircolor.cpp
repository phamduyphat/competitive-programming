...
vector<vector<int32_t>> adj(maxN), same (maxN);
vector<int32_t> root(maxN, 0);
queue<int32_t> Q;
/***********************************************************/
//Functions declarations:
void join (int32_t u, int32_t v)
{
    int32_t p = root[u], q = root[v];
    if (p == q)
        return ;
    if (same[p].size() < same[q].size()) swap(p, q);
    forv(v, same[q]) 
    {
        same[p].pb(v);
        root[v] = p;
...
void solve()
{
    ...
    while(!Q.empty())
    {
        ...
        int32_t last = adj[u].back();
        adj[u].pop_back();
        join(adj[u].back(), last);
    }
    vector<int32_t> ans(maxN, 0);
    int32_t res = 0;
    fr1(i, n)
    {
        if (!ans[root[i]])
            ans[root[i]] = ++res;
        cout << ans[root[i]] << endline;
    }
}
//Functions content:
...
/*
dsu
    với mọi u sao cho g[u].size > 0
                        q.push(u)
    while(!q.empty())
        lấy ra u;
        nếu q[u].size() > 1 bỏ qua
        xét đỉnh thuộc g[u];
            gán chúng cùng một đại diện root[];
            trộn các g[vi] vào g[vr]
                clear cái g[vi];
            nếu xuất hiện bậc > 2;
            thì push vào q
