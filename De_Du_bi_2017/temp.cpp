...
void Sieve(int32_t n, vector<int32_t> &pprime)
{
	pprime[0] = 0;
	pprime[1] = 1;
...
int64_t calculate_factors(int32_t &n, vector<int32_t> &p)
{
	if(n == 1)
		return 1;
	int64_t ans = 1;
	int32_t c = 1;
	for(int32_t prev = p[n], j = n / p[n]; j != 1;)
	{
		if (prev == p[j])
		{
			c ++;
