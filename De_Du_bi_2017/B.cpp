...
void Sieve(int32_t n, vector<bool> &pprime)
{
	pprime[0] = false;
	pprime[1] = false;
...
int32_t reversDigits(int32_t num)
{
    int32_t rev_num = 0;
    while (num > 0) {
...
bool check_reverse_is_a_prime(int32_t a, vector<bool> &p)
{
	if (p[reversDigits(a)])	
		return true;
	return false;
...
void solve(int64_t test_cases)
{
	...
	int32_t p, q;
	cin >> p >> q;
	vector<int32_t> dp(q + 1, 0);
	vector<bool> pprime(1e7 + 1, true);
	Sieve(1e7 + 1, pprime);
	FOR(i, 2, q)
	{
		dp[i] = dp[i - 1] + check_reverse_is_a_prime(i, pprime);
	}	
	cout << dp[q] - dp[p - 1] << endline;
}
//Functions content:
