...
bool validator(int64_t x, int32_t &n, int32_t &k, vector<int64_t> &dp, vector<int64_t> &cnt, vector<int64_t> &a)
{
    fr1(i, n)
    {
        dp[i] = INT64_MAX;
...
void solve()
{
    ...
    int64_t maxA = INT64_MIN;
    cin >> n >> k;
    vector<int64_t> a(n);
    unordered_map<int32_t, bool> m;
    forv(v, a)
    {
        cin >> v;
        if (v > maxA) maxA = v;
    ...
    if (n <= 100)
    {
        vector<vector<int64_t>> dp(k + 1, vector<int64_t> (n, INT64_MAX));
        fr(i, n - 1)
        {
            dp[1][i] = mul(a[i], i + 1);
        }
        ...
        FOR(i, 2, k)
            FOR(j, i - 1, n - 1)
            {
                FOR(k, 0, j - 1)
                {
                    if (dp[i - 1][k] == INT64_MAX)
                        continue;
                    dp[i][j] = min(dp[i][j], dp[i - 1][k] + mul(a[j], j - k));
                }
            }
        cout << dp[k][n - 1];
        return;
    ...
    vector<int64_t> dp(n + 1, 0), cnt(n + 1, 0);

    int64_t ll = 0, rr = INT64_MAX - 1;
    while(ll <= rr)
    {
        int64_t mid = MID;
        if (validator(mid, n, k, dp, cnt, a)) 
        {
            res = mid;
            rr = mid - 1;
        }
