...
struct segment_tree{
    ...
    int32_t build(int32_t id, int32_t ll, int32_t rr, vector<pair<int32_t, int32_t>> &a)
    {
        if (ll == rr)
        {
            return st[id] = a[ll].fi;
        }
        return st[id] = max(build(Lcon, a), build(Rcon, a));
    }
    int32_t get(int32_t id, int32_t ll, int32_t rr, int32_t u, int32_t v)
    {
        if (rr < u || ll > v)
            return INT32_MIN;
        if (u <= ll && rr <= v)
            return st[id];
        return max(get(Lcon, u, v), get(Rcon, u, v));
    }
};
/***********************************************************/
//Global variables below this line:
const int32_t maxN = 1e5 + 5;
int32_t lis[maxN], ma[maxN];
vector<pair<int32_t, int32_t>> a(maxN);    
/***********************************************************/
...
void solve()
{
    ...
    fr1(i, n)
    {
        int32_t f = i, l = n, pos = i, neg = i;
        while (f <= l)
        {
            int32_t mid = (f + l) / 2;
            if (st.get(1, 1, n, i, mid) == a[i].fi)
            {
                pos = mid;
                f = mid + 1;
            }
    ...
    fr1(i, n)
    {
        ma[i] = max(ma[i - 1], lis[a[i].se]);
    }
    while (q --)
    {
        int32_t k, f = 1, l = n, ans = 0;
        cin >> k;
        while (f <= l)
        {
            int32_t mid = (f + l) / 2;
