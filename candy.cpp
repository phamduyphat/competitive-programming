...
driver main(){
    ...
    cin >> n >> m;
    vector<vector<int32_t>> a(n + 1, vector<int32_t> (m + 1, 0)),
    dp(n + 1, vector<int32_t> (m + 1, 0));
    vector<bool> f(m + 1, false);
    vector<int32_t> k(n + 1, 0),
    b(m + 1, 0);
    ...
    fr1(i, n)
    {
        ...
        {
            if (!f[j])
                dp[i][j] = min(k[i - 1] + a[i][j] + b[j], dp[i - 1][j] + a[i][j] + b[j]);
            else 
                dp[i][j] = min(k[i - 1] + a[i][j] + b[j], dp[i - 1][j] + a[i][j]);
            if (dp[i][j] < temp)
            {
                k[i] = dp[i][j];
                temp = dp[i][j];
