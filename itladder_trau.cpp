...
void update2(int32_t id, int32_t l, int32_t r, int32_t pos, int32_t val);
...
int64_t getSum2Util(int32_t l, int32_t r, int32_t u, int32_t v, int32_t id)
{
	if (l >= u && r <= v) return node2[id].val;
	if (r < u || l > v) return 0;
	int32_t mid = (l + r) / 2;
	return getSum2Util(l, mid, u, v, 2 * id) + getSum2Util(mid + 1, r, u, v, 2 * id + 1);
}