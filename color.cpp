...
struct BFS{
    queue<int32_t> q;
    vector<bool> used;
    vector<int32_t> predecessor;
    BFS(int32_t &N, int32_t &u)
    {
        used.resize(N + 1, false);
        predecessor.resize(N + 1);
    }
    void bfs(int32_t &s, vector<vector<int32_t>> &adj)
    {
        q.push(s);
        used[s] = true;
        predecessor[s] = -1;
...
void solve()
{
    ...
    while(Q --)
    {
        ...
        tree.bfs(u, E);
        vector<int32_t> path;
        for(int32_t i = v; i != -1; i = tree.predecessor[i])
        {
            path.pb(i);
        }
