...
class BIT
{
    public:
        ...
        void update(int32_t id, int64_t val)
        {
            while(id > 0)
            {
                tree[id] += val;
...
driver main(){
    ...
    vector<vector<int32_t>> dp(n + 1, vector<int32_t> (n + 1, 0));
    BIT sol;
    fr1(i, n) {cin >> a[i]; dp[1][i] = 1;}
    FOR(i, 2, k)
    {
        forv(v, sol.tree) v = 0;
        fr1(j, n)
        {
            dp[i][j] = sol.query(a[j] + 1);
            sol.update(a[j], dp[i - 1][j]);
        }
    }
    fr1(i, n)
    {
