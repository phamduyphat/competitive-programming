...
bool check(int32_t x1, int32_t y1, int32_t x2, int32_t y2, vector<vector<bool>> &a)
{
    if (x2 - x1 == y2 - y1)
    {
        return false;
...
void solve()
{
    ...
    fr(i, n)
    {
        ...
        fr(j, n)
        {
            a[i][j] = (input[j] == 'B' ? true : false);
        }
    }
    map<pair<pair<int32_t, int32_t>, pair<int32_t, int32_t>>, bool> m;
    vector<pair<pair<int32_t, int32_t>, pair<int32_t, int32_t>>> rec;
    fr(x1, n)
        fr(y1, n)
        {
            if (not a[y1][x1])  
            ...
            FOR(x2, x1, n - 1)
                FOR(y2, y1, n - 1)
                {
                    if (not a[y2][x2] && (x1 == x2 && y1 == y2))
                    {
                        continue;
                    }
                    if (not check(x1, y1, x2, y2, a))
                        continue;
                    if (m.find(mp(mp(x1, y1), mp(x2, y2))) != m.end())
                        continue;
                    m.insert(mp(mp(mp(x1, y1), mp(x2, y2)), 1));
                    rec.pb(mp(mp(x1, y1), mp(x2, y2)));
                }
        }
    fr(i, rec.size() - 1)
        FOR(j, i + 1, rec.size() - 1)
        {
            auto &v1 = rec[i];
            auto &v2 = rec[j];
            if (v1.fi.se > v2.se.se || v1.se.se < v2.fi.se || v1.fi.fi > v2.se.fi || v1.se.fi < v2.fi.fi)
            {
                ans ++;
                ans %= MODULO;
            }
