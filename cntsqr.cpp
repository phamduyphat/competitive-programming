...
void calculate_all_substract(vector<int32_t> &input, int32_t &sz, unordered_map<int32_t, int32_t> &m)
{
    fr(i, sz)
        FOR(j, i + 1, sz - 1)
        {
            int32_t temp = input[j] - input[i];
...
void solve()
{
    ...
    unordered_map<int32_t, int32_t> mx, my;
    calculate_all_substract(x, v, mx);
    calculate_all_substract(y, h, my);
    forv(v, my)
    {
        if (mx.find(v.fi) == mx.end())
        {
            continue;
        }
        res += v.se * mx[v.fi];
