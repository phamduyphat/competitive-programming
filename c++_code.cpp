...
int32_t n, sts[] = {0, 1, 2, 4, 5, 8, 9, 10};
/***********************************************************/
...
driver main(){
	...
	cin >> n;
    vector<vector<int32_t>> a (4, vector<int32_t>(n + 1, 0)), f(n, vector<int32_t> (11));
	fr(i, 4) fr1(j, n) cin >> a[i][j];
	fr1(j, n) fr(st, 8) 
	{
		fr(i, 4) if (sts[st] & (1 << i)) f[j][sts[st]] += a[i][j];
