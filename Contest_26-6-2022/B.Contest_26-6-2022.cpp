...
vector<vector<int64_t>> multiplication(vector<vector<int64_t>> a, vector<vector<int64_t>> b, int64_t n)
{
    vector<vector<int64_t>> c(n, vector<int64_t> (n));
    fr(i,n) fr(j,n){
        c[i][j] = 0;
        fr(t, n) c[i][j] = (c[i][j] + (a[i][t] * b[t][j]) % MODULO) % MODULO;
    }
    return c;
}
vector<vector<int64_t>> exponential(vector<vector<int64_t>> a, int64_t n, int64_t length)
{
    if (n == 1) return a;
    vector<vector<int64_t>> temp = exponential(a, n / 2, length);
    temp = multiplication(temp, temp, length);
    if (n % 2 == 0) return temp;
    else return multiplication(temp, a, length);
}
/***********************************************************/
driver main(){
    ...
    while(T --)
    {
        ...
        fr(i, n)
            fr(j, n)
                cin >> b[i][j];
        a = exponential(a, 3, n);
        bool br = false, ok = false;
        fr(i, n)
        {
            fr(j, n)
                if(a[i][j] != b[i][j]) {cout << "NO" << endline; br = true; break;}
            if (br == true) {ok = true; break;} 
        }
        if (ok == true) continue;
        cout << "YES" << endline;
    }
