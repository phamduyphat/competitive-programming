...
int64_t getbit(int64_t i, int64_t x){return ((x >> i) & 1);}
/***********************************************************/
void solve()
{
	...
	cin >> N;
	vector<int64_t> pos;
	vector<uint64_t> acc = {0};
	int64_t sz = __builtin_popcountll(N);
	for(int64_t i = 0; i <= 15, sz > 0; i ++)
	{
		if (getbit(i, N))
		{
			sz --;
