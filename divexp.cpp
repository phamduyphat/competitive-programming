...
int64_t power(int32_t a, int32_t b, int32_t c) 
{
    if (a == 0 || a == 1) return a;
    if (b == 0) return 1;
    int64_t ans = 1;
    ...
    {
        ans = ans * a;
        if (ans > c) {ok = false; break;}
    }
    if (!ok) return c + 1;
    return ans;
}
int64_t shit(int32_t a, int32_t k)
{
    int32_t ans = 0;
    fr1(i, a)
        ans += (a / i - a / power(i, k, a));
    return ans;
}
/***********************************************************/
driver main(){
    ...
    int32_t ll, rr, k;
    cin >> ll >> rr >> k;
    cout << shit(rr, k) - shit(ll - 1, k) << endline;
    finish();
    return 0;
}
//functions content: