...
unordered_map<int32_t, int32_t> M[402];
...
void solve()
{
	...
	int32_t n, m;
	cin >> n >> m;
	vector<vector<int32_t>> A(n + 1, vector<int32_t> (m + 1, 0)), dp(n + 1, vector<int32_t> (m + 1, 0));
	fr1(i, n)
		fr1(j, m)
			cin >> A[i][j];	
	fr1(i, m)
	...
	fr1(i, n)
	{
		...
		{
			if (M[j].count(A[i][j]))
				dp[j][j] = max(dp[j][j], M[j][A[i][j]]);
			ans = max(ans, i - dp[i][j] + 1);
		}
		FOR(s, 2, m)
			for(int32_t l = 1, r = s; r <= m; l ++, r ++)
			{
				dp[l][r] = max(dp[l][r], max(dp[l][r - 1], dp[l + 1][r]));
				if (M[l].count(A[i][r]))
					dp[l][r] = max(dp[l][r], M[l][A[i][r]]);
				
				if (M[l].count(A[i][l]))
					dp[l][r] = max(dp[l][r], M[l][A[i][l]]);
				if (A[i][l] == A[i][r])
				{
					dp[l][r] = i + 1;
				}
				ans = max(ans, s * (i - dp[l][r] + 1));
			}
	}
	cout << ans << endline;
	return;	
