...
pair<int32_t, int32_t> ans; // size, pos;
/***********************************************************/
//Functions declarations:
void ktra(vector<int32_t> &check, vector<pair<int32_t, int32_t>> &inp)
{
	fr(i, check.size() - 1)
	{
		FOR(j, i + 1, check.size() - 1)
		{
			auto &v = inp[check[i]],
				 &k = inp[check[j]];
			if ((v.first < k.first && v.second < k.second) || (v.first > k.first && v.second > k.second))
				continue; 
			else	
				return;
		}
	...
	if (check.size() > 0)
		if (ans.first <= check.size())
		{
			ans.second = check[0];
			ans.first = check.size();
		}
}
void duyet(int32_t i, int32_t &n, vector<int32_t> check, vector<pair<int32_t, int32_t>> &input)
{
	if (i > n)
	{
		if (check.size() == 0)
...
void solve()
{
	...
	int32_t n, m;
	cin >> n >> m;
	vector<pair<int32_t, int32_t>> a = {{0, 0}};
	fr1(i, m)
	{
		pair<int32_t, int32_t> input;
		cin >> input.first >> input.second;
		a.pb(input);
	}
	vector<int32_t> check;
	duyet(1, m, check, a);
	cout << ans.second << endline;
}
//Functions content:

// #define task "easycode"
