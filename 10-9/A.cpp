...
int32_t check(vector<int32_t> &a, vector<int32_t> &check, int32_t prev)
{
	int32_t ans = 0;
	int32_t k = check[1] - check[0];
	FOR(i, 2, a.size() - 1)
	{
		check.pb(check[i - 1] + k);
...
void solve()
{
	...
	while(t --)
	{
		...
		vector<int32_t> temp = {a[0], a[1]};
		ans = min(ans, check(a, temp, 0));
		temp = {a[0] + 1, a[1]};
		ans = min(ans, check(a, temp, 1));
		temp = {a[0], a[1] + 1};
		ans = min(ans, check(a, temp, 1));
		temp = {a[0] + 1, a[1] + 1};
		ans = min(ans, check(a, temp, 2));
		temp = {a[0] - 1, a[1] - 1};
		ans = min(ans, check(a, temp, 2));
		temp = {a[0] - 1, a[1]};
		ans = min(ans, check(a, temp, 1));
