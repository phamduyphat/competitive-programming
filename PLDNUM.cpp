...
string R;
int32_t n;
int64_t save[50002][11][2][2];
/***********************************************************/
//Functions declarations:
int32_t f(int32_t i, int32_t p, int32_t ok1, int32_t ok2)
{
    if (save[i + 1][p][ok1][ok2] != -1) return save[i + 1][p][ok1][ok2];
    if (i > n / 2) return (ok1 || !ok2) && (p != 10);
    int32_t ans = 0;
    fr(j, 10)
    {
        if((p == 10 || abs(p - j) >= 5) && (ok1 || j <= R[i]))
        {
            ans += f(i + 1, (p == 10 && j == 0) ? 10 : j, ok1 || j < R[i], j > R[n - i - 1] - ok2);
            ans %= MODULO;
        }
    }
    return save[i + 1][p][ok1][ok2] = ans;
}
/***********************************************************/
//#define task ""
driver main(){
    ...
    reset(save, 255);
    cin >> R;
    if (R.size() % 2 == 0) R = string(R.size() - 1, '9');
    n = R.size();
    forv(v, R) v -= '0';
    cout << f(0, 10, 0, 0) << endline;
#ifndef task
    finish();
#endif
    return 0;
