...
void dfs(int32_t u, vector<vector<int32_t>> &adj, vector<int32_t> &num, vector<int32_t> &low, vector<int32_t> &visited, stack<int32_t> &st, vector<int32_t> &root, vector<vector<int32_t>> &ans, int32_t &index, int32_t &scc)
{
    num[u] = low[u] = ++index;
    st.push(u);
    visited[u] = 1;
    ...
    {
        if (num[v] == 0)
            dfs(v, adj, num, low, visited, st, root, ans, index, scc);
        if (visited[v] == 1)
...
driver main(){
    ...
    while(cin >> n)
    {
        if (n == 0)
            break;
        cin >> m;
        vector<vector<int32_t>> adj(n + 1), scc_ele(n + 1);
        vector<int32_t> num(n + 1, 0), low(n + 1, 0), visited(n + 1, 0), root(n + 1, 0);
        stack<int32_t> st;
        int32_t index = 0, scc = 0;
        fr1(i, m)
        {
            int32_t u, v;
            cin >> u >> v;
        ...
        {
            if (num[i] == 0) 
                dfs(i, adj, num, low, visited, st, root, scc_ele, index, scc);
        }
        fr1i (i, scc)
            if (scc_ele[i].size() > 2)
                //res += scc_ele[i].size();
                forv(u, scc_ele[i])
                    forv(v, adj[u]) if (root[u] == root[v]) res ++;
        cout << res << endline;
    }
#ifndef standard_input
    #ifndef task
