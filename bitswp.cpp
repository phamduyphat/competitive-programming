...
int32_t n;
vector<int64_t> v(100, 0), res(500, 0);
vector<pair<int32_t, int32_t>> l;
int16_t L;
/***********************************************************/
//Functions declarations:
int32_t count_bit(int64_t &n)
{
    int32_t ans = 0;
    fr(i, 64)
    {
...
void solve()
{
    ...
    if (l.size() == 50 && l.back().fi == 40)
    {
        ...
        fr(i, l.back().fi)
            res[l.back().se] += v[i];
        l.pop_back();
        res[l.back().se] += (v[38] + v[39]);
        l.back().fi -= 2;
    }   
    auto comp = [&](pair<int32_t, int32_t> &a, pair<int32_t, int32_t> &b)
    {
        return a.fi > b.fi;
    };
    fr(i, L + 1)    
