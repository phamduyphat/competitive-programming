...
void dfs(int32_t u, vector<vector<int32_t>> &adj, vector<int32_t> &visited, vector<int32_t> &low, vector<int32_t> &num, vector<int32_t> &val, vector<int32_t> &root, vector<int32_t> &a, vector<int32_t> &pos, stack<int32_t> &s, int32_t &t, int32_t &dag)
{
    low[u] = num[u] = ++ t;
    s.push(u);
    visited[u] = 1;
    forv(v, adj[u])
    {
        if (num[v] == 0)
        {
            dfs(v, adj, visited, low, num, val, root, a, pos, s, t, dag);
        }
        if (visited[v] == 1)
...
driver main(){
    ...
    int32_t dag = 0, t = 0;
    stack<int32_t> stack;
    vector<vector<int32_t>> adj(n + 1), dag_adj(n + 1);
    vector<int32_t> a(n + 1), root(n + 1, 0), visited(n + 1, 0), deg(n + 1, 0),
                    val(n + 1, 0), low(n + 1, 0), num(n + 1, 0), pos(n + 1, 0),
                    dp(n + 1, 0);
    fr1(i, m)
    {
        int32_t u, v;
    ...
    fr1(i, n)
        cin >> a[i];
    cin >> s;
    dfs(s, adj, visited, low, num, val, root, a, pos, stack, t, dag);
    stack.push(pos[s]);

    vector<int32_t> check(dag + 1, 0);
    fr1(i, n)
    {
        forv(v, adj[i])
        {
            if (pos[i] != pos[v]) 
                dag_adj[pos[i]].pb(pos[v]);
    ...
    fr1(i, dag)
    {
        ...
        {
            auto &v = dag_adj[i][j];
            while(check[v] && j < dag_adj[i].size()) 
                dag_adj[i].erase(dag_adj[i].begin() + j);
            check[v] = 1;
        }
    }
    fr1(i, dag)
