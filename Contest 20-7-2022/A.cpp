...
driver main(){
    ...
    deque<vector<vector<int32_t>>> dp(5, vector<vector<int32_t>> (k + 2, vector<int32_t> (2, INT32_MAX)));
    fr1(i, n) 
    {
        int32_t temp;
        cin >> temp;
    ...
    for(int32_t i = 2, temp3 = 2; temp3 <= n; i ++)
    {
        if (a[temp3] == 1)
        {
            dp[i][0][1] = dp[i - 1][0][1];
        ...
        fr1(j, k)
        {
            if (a[temp3] == 1)
            {
                dp[i][j][0] = min((dp[i - 1][j + 1][1] == INT32_MAX ? INT32_MAX : dp[i - 1][j + 1][1] + 1), dp[i - 1][j + 1][0]);
                dp[i][j][1] = min((dp[i - 1][j][0] == INT32_MAX ? INT32_MAX : dp[i - 1][j][0] + 1), dp[i - 1][j][1]);
            }
            else 
            {
                dp[i][j][0] = min((dp[i - 1][j + 1][1] == INT32_MAX ? INT32_MAX : dp[i - 1][j + 1][1] + 1), dp[i - 1][j + 1][0]);
        ...
        if (i >= 3) {vector<vector<int32_t>> temp1(k + 2, vector<int32_t> (2, INT32_MAX));dp.pop_front(); dp.pb(temp1); i --;}
        temp3 ++;
    }
    int32_t ans = INT32_MAX;
    fr(j, k + 1)
        fr(l, 2)
            ans = min(ans, dp[2][j][l]);
    cout << (ans == INT32_MAX ? -1 : ans) << endline;
#ifndef task
    finish();
#endif
    return 0;
