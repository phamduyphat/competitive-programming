...
driver main(){
    ...
    int32_t m, n, q, adkjflakdjfkladsjklfjasldfja;
    cin >> adkjflakdjfkladsjklfjasldfja;
    ...
    {
        input temp;
        cin >> temp.u >> temp.v >> temp.l >> temp.r;
        FOR(i, temp.u, temp.v)
            FOR(j, temp.l, temp.r)
            {
                x.pb(i);
        ...
        if (x.size() % 2 == 1)
        {
            int32_t x0 = x[x.size() / 2];
            int32_t y0 = y[x.size() / 2];
            forv(v, x) {ans += abs(x0 - v) % MODULO; ans %= MODULO;}
            forv(v, y) {ans += abs(y0 - v) % MODULO; ans %= MODULO;}
        }
        else 
        {
            int32_t x0 = (x[x.size() / 2] + x[x.size() / 2 - 1]) / 2;
            int32_t y0 = (y[x.size() / 2] + y[x.size() / 2 - 1]) / 2;
            forv(v, x) {ans += abs(x0 - v) % MODULO; ans %= MODULO;}
            forv(v, y) {ans += abs(y0 - v) % MODULO; ans %= MODULO;}
        }
        cout << ans << endline;
