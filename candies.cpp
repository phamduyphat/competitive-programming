...
void build(int32_t id, int32_t ll, int32_t rr, vector<int32_t> a)
{
    if (ll == rr)
    {
        st[id].val = a[ll];
...
void update(int32_t id, int32_t ll, int32_t rr, int32_t l, int32_t r)
{
    if (rr < l || ll > r) return;
    if (ll >= l && rr <= r) 
    {
    ...
    if (st[id].lazy) lazy(id);
    update(Lcon, l, r);
    update(Rcon, l, r);
    st[id].val = max(st[id * 2].val, st[id * 2 + 1].val);
    return;
}
int32_t query1(int32_t id, int32_t ll, int32_t rr, int32_t pos)
{
    if (ll == rr) return ll;
    if (st[id].lazy) lazy(id);
    if (st[id].val < pos) 
        return query1(Lcon, pos);
    else
        return query1(Rcon, pos);
}
int32_t query2(int32_t pos)
{
    if (st[1].val < pos) 
        return 0;
    else 
        return query1(1, 1, n, pos);
}
/***********************************************************/
driver main(){
    ...
    fr1(i, n) cin >> a[i];
    sort(a.begin() + 1, a.end(), greater<int32_t>());
    build(1, 1, n, a);
    cin >> m;
    while(m --)
    {
