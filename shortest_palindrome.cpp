...
const long long MODULO =	1e9+9277;
...
int64_t compute_hash_forward(string const &s)
{
    const int32_t p = 31;
    ...
    forv(v, s)
    {
        hash_value += Modular_multiplication(v - 'a' + 1, p_pow);
        hash_value %= MODULO;
        p_pow = mul(p_pow, p);
    }
    return hash_value;
}
int64_t compute_hash_backward(string const &s)
{
    const int32_t p = 31;
    int64_t hash_value = 0;
    int64_t p_pow = 1;
...
{
    return compute_hash_forward(s.substr(0, s.size() / 2)) == compute_hash_backward(s.substr(s.size() - s.size() / 2, s.size() / 2));
}
pair<int32_t, int8_t> find_palindrome_pre_app(string &s) //size, pre or append
{
    pair<int32_t, int8_t> res = {0, -1};
    string temp;
    fri(l, s.size())
    {
        temp += s[l];
    ...
    FORDI(r, s.size() - 1, 0)
    {
        temp += s[r];
        if (check_palindrome(temp))
            res = (res.first > s.size() - r ? res : (pair<int32_t, int8_t>)make_pair(s.size() - r, 2));
    }
    return res;
}
void shortest_palindrome(string &s)
{
    string builder = "";
    ...
    auto temp = find_palindrome_pre_app(s);
    int32_t v = s.size();
    if (temp.se == 1)
    {
        FORD(i, v - 1, v - temp.fi)
            cout << s[i];
        cout << s << endline;
    }
    elif (temp.se == 2)
