...
driver main(){
    ...
    str a;
    cin >> n >> m >> a;
    vector<int64_t> hash_table(m), ans(m);
    vector<bool> used(m, false);
    fr(i, m)
    {
        string input;
        cin >> input;
        hash_table[i] = Compute_hash(input);
    }
    fr(i, m)
    {
        string test_case = a.substr(i * (n / m), n / m);
        int32_t temp = Compute_hash(test_case);
        FORD(j, m - 1, 0)
        {
            if (temp == hash_table[j] && !used[j])
            {
                ans[i] = j;
                used[j] = true;
                break;
