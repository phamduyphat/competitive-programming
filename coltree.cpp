...
void dfs(int32_t u, int32_t prev, vector<vector<int32_t>> &adj, vector<int32_t> &ans, vector<int32_t> &color, vector<set<int32_t>> &s)
{
    s[u].insert(color[u]);
    forv(v, adj[u])
    {
...
driver main(){
    ...
    vector<set<int32_t>> s(n + 1);
    vector<int32_t> color(n + 1, 0), ans(n + 1, 0);
    fr1(i, n - 1)
    {
        int32_t u, v;
        cin >> u >> v;
