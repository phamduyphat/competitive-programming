...
vector<vector<pair<int8_t, str>>> arr(102, vector<pair<int8_t, str>>(10010, {-1, ""}));
pair<int8_t, str> dp(int32_t i, int32_t prev, string s, vector<int32_t> &h, vector<int32_t> &t, int32_t &q, int32_t &n)
{
	if (i == n)
	{
		return mp(prev == q, s);
	}
	if (arr[i][prev].first != -1)
	{
		return arr[i][prev];
	}
	if (prev > q)
	...
	arr[i][prev] = dp(i + 1, prev + h[i], s + 'H', h, t, q, n);
	if (arr[i][prev].first == 1)
	{
		return arr[i][prev];
	}
	arr[i][prev] = dp(i + 1, prev + t[i], s + 'T', h, t, q, n);
...
void solve(int64_t test_cases)
{
	...
	auto temp = dp(0, 0, "", h, t, q, n);
	if(temp.first == 1)
	{
		cout << "Yes" << endline;
		cout << temp.second << endline;
	}
	else
	{
		cout << "No" << endline;
