...
driver main(){
    ...
    cin >> k;
    if (k == 0) {cout << 0 << endline; finish(); return 0;}
    int64_t temp = k;
    for(int64_t i = 1; i <= abs(k); i += 2)
    {
        if(checkIsSquare(temp)) {cout << square(temp) << endline; finish(); return 0;}
        temp += i;
    }
    cout << "none" << endline;
    finish();
