...
void solve(int32_t &a, char dec, char inc)
{
    iota(id, id + n, 1);
    ...
    for(int32_t i = 0; i < n; ++ i) if (a[i] > i + 1)
        for(int32_t j = 0; j < a[i] - i - 1; ++ j) ans.pb( id[i], dec);
    for(int32_t i = n - 1; i >= 0; i --) if (a[i] < i + 1)
        for(int32_t j = 0; j < i + 1 - a[i]; j ++)
            ans.pb(id[i], inc);
}
/***********************************************************/
//#define task ""
driver main(){
    ...
    cin >> n;
    for(int32_t i = 0; i < n; i++ ) cin >> r[i] >> c[i];
    solve(r, 'U', 'D');
    solve(c, 'L', 'R');
    forv(x, ans) cout << x.fi << spaceuk << x.se << endline;
#ifndef task
    finish();
#endif
    return 0;
...
/*  giả sử (ri, idi)    ^   r'i
            ...
            trái bé hơn đi trước
            phải lớn hơn đi trước
            => ko bao h đè lên nhau
*/