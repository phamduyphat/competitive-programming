...
int64_t save[31][16][4], N[31], H[4][31];
/***********************************************************/
//Functions declarations:
int64_t dp(int32_t i, int32_t ok, int32_t r){
    if (i == 31) 
    {
        return r == 0 && !ok;
    }
    int64_t &ans = save[i][ok][r];
    if (ans != -1) return ans;
    ans = 0;
    fr(a, 2)
    {
        fr(b, 2)
            fr(c, 2)
            {
                int32_t d = (N[i] - a - b - c + 6 - r) % 2;
                int32_t j = a + 2 * b + 4 * c + 8 * d;
                int32_t okk = 0;
                fr(t, 4)
                {
                    okk |= (getbit(t, j) > H[t][i] - getbit(t, ok)) << t;
                }   
                ans += dp(i + 1, okk, (r + a + b + c + d) / 2);
                ans %= MODULO;
            }
    }
    return ans;
...
/*
    {
        ...
        x3 = .... 
        x4 = ....
        dp(i, ok, r)            ok: xi đã lớn hơn hoặc bằng Li chưa;
                                r : nhớ(phép cộng có nhớ);

        = dp (n + A - a - b - c + 30) % 2    
        = dp(i + 1, ok, (a + b + c + d + r) / 2)
        N = .....
    }


    Lưu ý xây dựng từ hàng thấp lên hàng cao
*/