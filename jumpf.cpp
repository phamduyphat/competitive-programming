...
int64_t getf(int32_t i, int32_t j, vector<vector<int64_t>> &dp, vector<int16_t> &a)
{
    if (dp[i][j] != LLONG_MAX)
        return dp[i][j];
    if (i == n) return dp[i][j] = 0;
    int64_t ans = LLONG_MAX;
    if (i > j) ans = min (ans, 1LL * a[i - j] + getf(i - j, j, dp, a));
    if (i + j < n) ans = min (ans, 1LL * a[i + j + 1] + getf(i + j + 1, j + 1, dp, a));
    return dp[i][j] = ans;
}
/***********************************************************/
//#define task "jumpf"
driver main(){
    ...
    cin >> n;
    vector<int16_t> a(n + 1);
    vector<vector<int64_t>> dp(n + 1, vector<int64_t> (n - 1, LLONG_MAX));
...
/*
    f[i][j] = x:
        chi phí x để đên n nếu đang ở ô i, vừa thực hiện bước nhảy độ dài j
    
    đích f(n, ?) = 0;
    kết quả: a[2][1];


    tính f[i][j] theo thứ tự:
        for(j = n -> 1) 
            for(i = 1 -> n)
    ...
    f(i, j) = INT_MAX
    int32 get f(int32 i, int32 j)
    {
        if (f(i, j) != chưa tính)
            return f(i, j);
        if (i == n) return f[i][j] = 0;
        ans = LLONG_MAX;
        if (i > j) ans <- min(ans, a[i - j] + get(i - j, j));
        if (i + j < n) ans <- min(ans, a[i + j + 1] + get(i + j + 1, j + 1));
        return f[i][j] = ans;
    }
*/