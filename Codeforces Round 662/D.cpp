...
void solve()
{
	...
	int32_t n, m;
	cin >> n >> m;
	vector<vector<char>> a(n + 2, vector<char> (m + 2, 1));
	vector<vector<int64_t>> dpl(n + 2, vector<int64_t> (m + 2, 1)), 
							dpr(n + 2, vector<int64_t> (m + 2, 1)),
							dpu(n + 2, vector<int64_t> (m + 2, 1)),
							dpd(n + 2, vector<int64_t> (m + 2, 1));
	fr1(i, n)
	{
		str s;
		cin >> s;
	...
	fr1(i, n)
		fr1(j, m)
		{
			...
			if (a[i][j] == a[i - 1][j])
			{
				dpl[i][j] = min(dpl[i - 1][j] + 1, dpl[i][j]);
			}
			else
			{
				dpl[i][j] = 1;
	...
	FORD(i, n, 1)
		FORD(j, m, 1)
		{
			...
			if (a[i][j] == a[i + 1][j])
			{
				dpr[i][j] = min(dpr[i + 1][j] + 1, dpr[i][j]);
			}
			else
			{
				dpr[i][j] = 1;
	...
	fr1(i, n)
...

		{
			...
			if (a[i][j] == a[i - 1][j])
			{
				dpu[i][j] = min(dpu[i - 1][j] + 1, dpu[i][j]);
			}
			else
			{
				dpu[i][j] = 1;
	...
	FORD(i, n, 1)
		fr1(j, m)
		{
			...
			if (a[i][j] == a[i + 1][j])
			{
				dpd[i][j] = min(dpd[i + 1][j] + 1, dpd[i][j]);
			}
			else
			{
				dpd[i][j] = 1;
	...
	fr1(i, n)
		fr1(j, m)
		{
			//if (min(dpl[i][j], dpr[i][j]) > 2)
			//	cout << "i, j: " << i << spaceuk << j << endline;
			ans += min(min(dpl[i][j], dpr[i][j]), min(dpu[i][j], dpd[i][j]));
		}
	cout << ans << endline;
}
//Functions content:
