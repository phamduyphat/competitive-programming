...
Point projection(Point v, Point u) { // Chiáº¿u vector v lÃªn vector u
    double scalar = (v * u) / (u * u);
    return u * scalar;
}
Point projection(Point p, Point a, Point b, Point c) { // Chiáº¿u Ä‘iá»ƒm p lÃªn máº·t pháº³ng ABC
...
double dist(Point p, Point b, Point a) { // Khoáº£ng cÃ¡ch tá»« p tá»›i Ä‘Æ°á»ng tháº³ng AB
    Point _p = p;
    p = p - a;
