...
void solve(int64_t test_cases)
{
	...
	int32_t n, w;
	cin >> n >> w;
	vector<pair<int32_t, int32_t>> in = {{0, 0}};
	fr1(i, n)
	{
		pair<int32_t, int32_t> a;	
		cin >> a.first >> a.second;
		in.pb(a);
	}	
	vector<vector<uint64_t>> dp(n + 1, vector<uint64_t> (1e5 + 10, INT64_MAX));
	dp[0][0] = 0;
	fr1(i, n)
	{
		fr(j, 1e5 + 10)
		{
			...
			if (j >= in[i].second)
				dp[i][j] = min(dp[i - 1][j - in[i].second] + in[i].first, dp[i][j]);
		}
	}
	FORD(i, dp[n].size() - 1, 0)
	{
		if (dp[n][i] <= w)
		{
			cout << i << endline;
