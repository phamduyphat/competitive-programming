...
void solve(int64_t test_cases)
{
	...
	int32_t n;
	cin >> n;
	vector<array<int32_t, 3>> in = {{0, 0, 0}};
	vector<vector<int32_t>> a = {{1, 2},
								{0, 2},
								{0, 1}};
	fr1(i, n)
	{
	...
	FOR(i, 2, n)
	{
		...
		{
			fr(k, 2)
				dp[i][j] = max(dp[i][j], dp[i - 1][a[j][k]] + in[i][j]);
		}	
	}
	cout << *max_element(dp[n].begin(), dp[n].end()) << endline;
}
//Functions content:
