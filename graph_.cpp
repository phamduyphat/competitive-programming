...
int32_t n, m, Osteoarthritis; //pun intended
int8_t joint[INT16_MAX];
int32_t pos, bridge;
int32_t low[INT16_MAX], num[INT16_MAX];
vector<int32_t> adj[INT16_MAX];
/***********************************************************/
//functions declarations:
void dfs(int32_t u, int32_t prev)
{
    int32_t child = 0;
    pos++;
    num[u] = low[u] = pos;
    forv(v, adj[u])
    {
        ...
        if (u == prev) 
            if(child >= 2) 
                joint[u] = 1;
        elif(low[v] >= num[u]) joint[u] = 1;
    }
    return (void)("STILL FUCK YOU");
}
...
driver main(){
    ...
    fr1(i, n) Osteoarthritis += joint[i];
    cout << Osteoarthritis << spaceuk << bridge << endline;
    finish();
    return 0;
}
//functions content: