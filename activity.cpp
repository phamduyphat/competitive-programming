...
driver main(){
	...
	while(T --)
	{
		int32_t m, n, k, a, b; 
		cin >> m >> n >> k >> a >> b; 
		int64_t temp = min(m / a, n / b),
				left = m - temp * a + n - temp * b;
		if (left >= k) {cout << temp << endline; continue;}
		int64_t c = (k - left) / (a + b);
		int64_t ans = temp - c - ((k - left) % (a + b) != 0);
		if (ans < 0) cout << 0 << endline;
