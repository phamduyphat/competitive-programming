...
vector<int32_t> adj[5002], iadj[5005];
int32_t n, m, w, b;
...
driver main(){
    ...
    fr1(x, n)
    {
        ...
        fr(i, topo.size())
        {
            ...
            {
                --deg[v];
                if (deg[y] == 0) topo.pb(y);
            }             
        }
    }
    
    vector<int32_t> itopo = topo;
    reverse(itopo.begin(), itopo.end());
    forv(x, topo) forv(y, itopo)
    {
        if (f[x][y])
        {
            g[x][y] = 1e9;
            forv(xx, adj[x])
                if(f[xx][y] == false) f[x][y] = min(f[x][y], f[xx][y] + 1);
            forv(yy, adj[y])
                if(f[x][yy] == false)   f[x][y] = min(f[x][y], f[x][yy] + 1);
