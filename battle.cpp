...
void solve()
{
    ...
    sortall(a);
    sortall(b);
    vector<vector<vector<int64_t>>> prev(n + 1, vector<vector<int64_t>> (m + 1, vector<int64_t> (k + 1, 0))), 
                                    dp(n + 1, vector<vector<int64_t>> (m + 1, vector<int64_t> (k + 1, 0)));
    fr1(i, n)
        fr1(j, m)
        {
            fr1(l, k)
            {
                if (a[i - 1] > b[j - 1])
                    prev[i][j][l] = (l == 1 ? 1 : dp[i - 1][j - 1][l - 1]);
                dp[i][j][l] = addition(addition(dp[i][j - 1][l] ,- dp[i - 1][j - 1][l]), addition(dp[i - 1][j][l], prev[i][j][l]));
            }
        }
    cout << dp[n][m][k];
}
