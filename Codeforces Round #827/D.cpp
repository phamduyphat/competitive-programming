...
void solve(int64_t test_cases)
{
	...
	int64_t maxid = 0;
	cin >> n;
	vector<vector<int32_t>> a(1001);
	fr1(i, n)
	{
		int32_t temp;	cin >> temp;
		a[temp].pb(i);
	}
	vector<vector<int32_t>> pair(1001, vector<int32_t> (1001, false));
	fr(i, 1001)
		fr(j, 1001)
		{
			if (gcd(i, j) == 1)
	...
	fr1(i, 1000)
	{
		forv(j, pair[i])	
		{
			if (not a[i].empty() && not a[j].empty())
			{
				ans = max(ans, 1LL * a[i].back() + a[j].back());
			}
		}
	}
	cout << (ans == 0 ? -1 : ans) << endline;
}
//Functions content:
