...
void task1(vector<vector<int64_t>> &dp, vector<input> a, int64_t i, int64_t t)
{
    dp[i][1] = max(dp[i][1], dp[i - 1][1] + a[i].r - a[i - 1].r);
    if (a[i].l - a[i - 1].r >= t) dp[i][1] = max(dp[i][1], dp[i - 1][0] + a[i].r - a[i - 1].r - t);
    if (a[i].l - a[i - 1].r >= t + t) dp[i][0] = max(dp[i][0], dp[i - 1][0] + a[i].l - a[i - 1].r - t - t);
    if (a[i].l - a[i - 1].r >= t) dp[i][0] = max(dp[i][0], dp[i - 1][1] + a[i].l - a[i - 1].r - t);
}
void task0(vector<vector<int64_t>> &dp, vector<input> a, int64_t i, int64_t t)
{
    if (a[i].l - a[i - 1].r >= t) dp[i][0] = max(dp[i][0], dp[i - 1][1] + a[i].l - a[i - 1].r - t);
    if (a[i].l - a[i - 1].r >= t + t) dp[i][0] = max(dp[i][0], dp[i - 1][0] + a[i].l - a[i - 1].r - t - t);
}
...
driver main(){
    ...
    sort(a.begin(), a.end(), [&](const input &a, const input &b)
    {
        return a.l < b.l;
    });
    ...
    cout << max(dp[n - 1][1] + d - a[n - 1].r, dp[n - 1][0] + d - a[n - 1].r - t) << endline;
#ifndef task
    finish();
#endif
    return 0;
