...
driver main(){
    ...
    {
        input temp;
        cin >> temp.fi >> temp.se >> temp.th;
        if (temp.fi == 'N')
            a.pb({temp.se, temp.th});
        else 
            b.pb({temp.se, temp.th});
        xyz.pb(temp);
    }
    map<pair<int32_t, int32_t>, int32_t> t_block;
    vector<pair<pair<int32_t, int32_t>, pair<int32_t, int32_t>>> events;
    fr(i, a.size())
        fr(j, b.size())
        {
            if (a[i].fi < b[j].fi || a[i].se > b[j].se)
                continue;
            events.pb({{i, j}, {b[j].se - a[i].se, a[i].fi - b[j].fi}});
        }
    sort(events.begin(), events.end(), [&](const pair<pair<int32_t, int32_t>, pair<int32_t, int32_t>> &a, const pair<pair<int32_t, int32_t>, pair<int32_t, int32_t>> &b)
    {
        int32_t tempa = min(a.se.fi, a.se.se);
        int32_t tempb = min(b.se.fi, b.se.se);
        return tempa < tempb;
    });
    forv(v, events)
    {
        pair<int32_t, int32_t> &tempa = a[v.fi.fi];
        pair<int32_t, int32_t> &tempb = b[v.fi.se];
        if (t_block.find(tempa) == t_block.end() && t_block.find(tempb) == t_block.end())
        {
            if (v.se.fi < v.se.se)
            {
                t_block.insert({tempb, v.se.se});
            }
            elif (v.se.fi > v.se.se)
            {
                t_block.insert({tempa, v.se.fi});
            }
        }
    }
    forv(v, xyz)
    {
        if (t_block.find({v.se, v.th}) == t_block.end()) cout << "Infinity" << endline;
        else cout << t_block.find({v.se, v.th}) -> second << endline;
    }
#ifndef task
    finish();
#endif
...
/*
    Sinh mọi khả năng cắt nhau
    mỗi sự kiện cắt nhau: tđ cắt i1, i2, t1, t2
    xử lí sự kiện cắt : 2 robot tham gia đã bị chặn chưa
                        t1 < t2: i1 chưa bị chặn, i2 chưa bị chặn --> i2 bị chặn ở t2
                        cập nhật mảng thdi bị chặn t_block[] của mỗi robot     
    sau xử lí nhx con nào chưa bị chặn --> INFINITY
