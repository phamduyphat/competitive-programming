...
void solve()
{
    ...
    int32_t n, C;
    cin >> n >> C;
    vector<int64_t> maximum_power(C + 1, 0);
    fr1(i, n)
    {
        int32_t c, d, h;
        cin >> c >> d >> h;
        maximum_power[c] = max(maximum_power[c], 1LL * d * h);
    }
    for(int32_t c = 1; c <= C; c ++)
        for(int32_t x = c; x <= C; x += c)
        {
            maximum_power[x] = max(maximum_power[x], 1LL * maximum_power[c] * (x / c));
        }
    fr1(c, C)
    {
        maximum_power[c] = max(maximum_power[c], maximum_power[c - 1]);
    }
    int32_t m;
    cin >> m;
    while(m --)
    {
        ...
        int64_t H;
        cin >> D >> H;
        auto id = upper_bound(maximum_power.begin(), maximum_power.end(), D * H);
        auto minC = id - maximum_power.begin();
        if (minC > C) 
            cout << -1;
        else    
            cout << minC;
