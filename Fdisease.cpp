...
/*
    sắp xếp bò theo X;
    xét mọi cặp x(i), t(i) với x(i + 1), t(i + 1) với t(i) != t(i + 1):
        chọn R lớn nhất có thể : R = min(x(i + 1), x(i));
        cứ đi từ trái sang phải là sẽ xong;
        nếu như con trước 
        chỉ khi gặp con bò gần khoảng cách lây nhiễm trc 
