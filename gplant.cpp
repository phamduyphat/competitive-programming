...
{
    struct segment_tree{
        int64_t val = 0, lazy = 0;
    }*st;
    seggs(int32_t n)
    {
        delete [] this -> st;
        this -> st = new segment_tree[4 * n];

    }
    void lazy(int32_t id, int32_t ll, int32_t rr)
    {
        this -> st[id].val += this -> st[id].lazy * (rr - ll + 1);
        if (ll != rr)
        {
            this -> st[id << 1].lazy += this -> st[id].lazy;
            this -> st[id << 1 | 1].lazy += this -> st[id].lazy;
        }
        this -> st[id].lazy = 0;
    }
    int64_t update(int32_t id, int32_t ll, int32_t rr, int32_t l, int32_t r, int32_t diff)
    {
        if (this -> st[id].lazy)
            lazy(id, ll, rr);
        if (r < ll || rr < l)
        ...
        if (ll >= l && rr <= r)
        {
            this -> st[id].lazy += diff;
            lazy(id, ll, rr);
            return st[id].val;
        }
        st[id].val = update(Lcon, l, r, diff) + update(Rcon, l, r, diff);
    }
    int64_t get(int32_t id, int32_t ll, int32_t rr, int32_t l, int32_t r)
    {
        if (this -> st[id].lazy)
            lazy(id, ll, rr);
        if (r < ll || rr < l)
            return 0;
        if (ll >= l && rr <= r)
            return this -> st[id].val;
        return get(Lcon, l, r) + get(Rcon, l, r);
    }
};
