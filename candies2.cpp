...
int64_t minFibo(vector<int64_t> vals, int64_t n)
{
    int64_t res = 0;
    for(; n; res ++)
    {
        auto it = upper_bound(vals.begin(), vals.end(), n);
        n -= *prev(it);
    }
    return res;
}
void solve(int32_t n, int32_t k){
    int64_t ans = 0;
    fr1(i, n)
    {
        if(minFib[i] >= k) ans += minFib[i];
    }
    cout << ans << endline;
}
/***********************************************************/
driver main(){
    ...
    minFib = new int64_t[maxN + 1];
    for(int64_t f0 = 1, f1 = 1; f1 <= maxN; swap(f1, f0), f1 += f0)
    {
        vals.pb(f1);
    }
    fr1i(i, maxN)
    {
        minFib[i] = minFibo(vals, i);
    }
    fri(i, T)
    {
        solve(input[i].fi, input[i].se);
    }
    delete [] minFib;
    finish();
    return 0;
