...
Priority:
    ...
    Nếu ko xếp được
        -1;
    đơn giản hoá : ko ưu tiên đội 1
        a1  -> an; P, Q:
    Tìm 1 số đội sao cho Tổng <= P                      lưu vết (để truy vết)
                         Tổng S hàng mã
    nếu total - S <= Q -> OK;   

    ưu tiên đội 1 ở hàng 1
    xet đội từ 2 -> n;
    Tìm số đội sao cho S <= Q && S -> max | truy vêt;
    --> save = a[i];
    a[1] = Q + 1;
    solve(Q);
    a[1] = save;
    ...
    used[]
    ngc lại (Total - S > P) 
        tìm 1 số đội (không kể đội 1) sao cho tổng S <= P && S -> max;
        nếu Total - S <= Q : 1;
        ngc lại -1;



    => cho dãy a(1 -> n) chọn tập phần tử sao cho tổng S <= M && S -> max           lưu vết


    int32_t knapsack (int32_t limit) 
                         trả về tổng lớn nhất <= limit
                            sửa mảng used
                            xếp vào hàng 2;
                    
    f[] <- -1;
    //qhđ -> ans = tổng max;
    //trong truy vết từ án, đánh dấu vào used
    return ans;
