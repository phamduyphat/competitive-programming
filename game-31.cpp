...
bool check_vertical(pair<int32_t, int32_t> &a, pair<int32_t, int32_t> &b) // 1
{
...
bool check_horizontal_left(pair<int32_t, int32_t> &a, pair<int32_t, int32_t> &b) // 2
{
    if (a.first == b.first - 1 && a.second == b.second)
        return true;
    return false;
}
bool check_horizontal_right(pair<int32_t, int32_t> &a, pair<int32_t, int32_t> &b) // 3
{
    if (a.first == b.first + 1 && a.second == b.second)
        return true;
    return false;
...
bool check_diagonal_left_downward(pair<int32_t, int32_t> &a, pair<int32_t, int32_t> &b) // 4
{
    if (a.first == b.first - 1 && a.second == b.second - 1)
        return true;
    return false;
}
bool check_diagonal_right_downward(pair<int32_t, int32_t> &a, pair<int32_t, int32_t> &b) // 5
{
    if (a.first == b.first + 1 && a.second == b.second - 1)
        return true;
    return false;
}
bool check_diagonal_left_upward(pair<int32_t, int32_t> &a, pair<int32_t, int32_t> &b) // 6
{
    if (a.first == b.first - 1 && a.second == b.second + 1)
        return true;
    return false;
}
bool check_diagonal_right_upward(pair<int32_t, int32_t> &a, pair<int32_t, int32_t> &b) // 7
{
    if (a.first == b.first + 1 && a.second == b.second + 1)
        return true;
    return false;
}
bool check_win(vector<pair<int32_t, int32_t>> &a, int32_t state, pair<int32_t, int32_t> &coord, int32_t cunt, int32_t &k)
{
    if (cunt == k)
        return true;
    bool res = false;
    if (state == -1)
        fr(i, a.size())
        {
            if (check_vertical(a[i], coord))
            {
                vector<pair<int32_t, int32_t>> temp = a;
                temp.erase(temp.begin() + i);
                res = check_win(temp, 1, a[i], cunt + 1, k);
                if (res == true)
                    break;
            }
            if (check_horizontal_left(a[i], coord))
            {
                vector<pair<int32_t, int32_t>> temp = a;
                temp.erase(temp.begin() + i);
                res = check_win(temp, 2, a[i], cunt + 1, k);
                if (res == true)
                    break;
            }
            if (check_horizontal_right(a[i], coord))
            {
                vector<pair<int32_t, int32_t>> temp = a;
                temp.erase(temp.begin() + i);
                res = check_win(temp, 3, a[i], cunt + 1, k);
                if (res == true)
                    break;
            }
            if (check_diagonal_left_downward(a[i], coord))
            {
                vector<pair<int32_t, int32_t>> temp = a;
                temp.erase(temp.begin() + i);
                res = check_win(temp, 4, a[i], cunt + 1, k);
                if (res == true)
                    break;
            }
            if (check_diagonal_right_downward(a[i], coord))
            {
                vector<pair<int32_t, int32_t>> temp = a;
                temp.erase(temp.begin() + i);
                res = check_win(temp, 5, a[i], cunt + 1, k);
                if (res == true)
                    break;
            }
            if (check_diagonal_left_upward(a[i], coord))
            {
                vector<pair<int32_t, int32_t>> temp = a;
                temp.erase(temp.begin() + i);
                res = check_win(temp, 6, a[i], cunt + 1, k);
                if (res == true)
                    break;
            }
            if (check_diagonal_right_upward(a[i], coord))
            {
                vector<pair<int32_t, int32_t>> temp = a;
                temp.erase(temp.begin() + i);
                res = check_win(temp, 7, a[i], cunt + 1, k);
                if (res == true)
                    break;
            }
        }
...
void solve()
{
    ...
    int32_t h, w, k;
    cin >> h >> w >> k;
    vector<vector<bool>> inp(w + 1, vector<bool> (h + 1, false));
    vector<pair<int32_t, int32_t>> A, B;
    fr1(i, h * w)
    {
        int32_t col;
        cin >> col;
        fr1(j, inp[col].size() - 1)
        {
            if (inp[col][j] == true)
                continue;
            inp[col][j] = true;
            pair<int32_t, int32_t> v = mp(col, j);
            if (i % 2 == 1)
            {
                if (check_win(A, -1, v, 1, k))
                {
                    cout << "A" << spaceuk << i;
                    return;
                }
