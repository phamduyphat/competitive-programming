...
void solve()
{
	...
	while(t --)
	{
		str s;
		cin >> s;
		vector<pair<char, int32_t>> inp(s.size());
		fri(i, s.size())
		{
			inp[i].first = s[i];
			inp[i].second = i + 1;
		...
		char tempa = inp[0].first, tempb = inp[s.size() - 1].first;
		sortall(inp);
		int32_t ll, rr;
		if (tempa <= tempb)
		{
			...
			FOR(i, ll, rr - 1)
			{
				sum += inp[i + 1].first - inp[i].first;
			}
			cout << sum << spaceuk << rr - ll + 1 << endline;
			cout << 1 << spaceuk;
			FOR(i, ll, rr)
			{
				if (inp[i].second == 1 || inp[i].second == s.size())	
					continue;
				cout << inp[i].second << spaceuk;
			}
			cout << s.size();
			cout << endline;
		}
		else
		{
			...
			cout << sum << spaceuk << ll - rr + 1 << endline;
			cout << 1 << spaceuk;
			FORD(i, ll, rr)
			{				
				if (inp[i].second == 1 || inp[i].second == s.size())	
