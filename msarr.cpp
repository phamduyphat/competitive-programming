...
struct dsu{
    ...
    void make_set(int64_t &n, vector<int64_t> &a)
    {
        fr1(i, n)
        {
            parent[i] = i;
    ...
    void union_set(int64_t a, int64_t b)
    {
        a = find_set(a);
        b = find_set(b);
        if (a != b)
...
void solve()
{
    ...
    int64_t n;
    cin >> n;
    vector<int64_t> input = {0}, check(n), res;
    vector<bool> add(n + 2, false);
    fr1(i, n)
    {
        int64_t temp;
        cin >> temp;
    ...
    int64_t maxAns = 0;
    dsu ans(n);
    ans.make_set(n, input);
    FORD(i, check.size() - 1, 0)
    {
        add[check[i]] = true;
        
        if (add[check[i] + 1] && check[i] != n)
            ans.union_set(check[i] + 1, check[i]);
        if (add[check[i] - 1] && check[i] != 1)
            ans.union_set(check[i], check[i] - 1);

        maxAns = max(maxAns, ans.sz[ans.find_set(check[i])]);
