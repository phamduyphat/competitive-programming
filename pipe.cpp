...
//Functions content:
/*
    signma xđích + sigma ynguồn - sigma x nguồn - sigma y đích
    (x, y, id, typ)
                0: nguồn
                1: đích
    sắp xếp: x tăng, x == nguồn trước;
            gặp nguồn: lưu vào S
            gặp đích: tìm trong S cái nguồn có Y bé nhất >= Y đích
        => dùng mảng lưu các sự kiên + S: set -> lowerbound() tìm cho nhanh;
*/