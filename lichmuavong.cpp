...
int32_t calc(int32_t &n, int32_t &m, vector<vector<char>> &input)
{
    vector<int32_t> dp(1 << m, MODULO),
                    temp(1 << m);
    dp[0] = 0;
    fr(i, n)
    {
            ...
            fr(k, 1 << m)
            {
                if (k & (1 << j))
                {
                    if (input[i][j] == '.')
                        ...
                        temp[k] = min(dp[k], dp[k ^ (1 << j)] + 1);
                }
                else
                {
                    temp[k] = min(dp[k], dp[k ^ (1 << j)]);
                    if (input[i][j] == '#' && 
                        (j == 0 || (k & (1 << (j - 1))) || 
                        input[i][j - 1] == '.'))
                            temp[k]++;
