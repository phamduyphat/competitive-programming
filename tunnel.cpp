...
struct dsu{
    ...
    bool join_set(int32_t a, int32_t b)
    {
        a = find_set(a);
        b = find_set(b);
        if (a != b)
...
pair<int32_t, int32_t> x[maxN], y[maxN], z[maxN];
/***********************************************************/
...
void solve()
{
    ...
    #define sortinfo(a, n) sort(a + 1, a + 1 + n)
    sortinfo(x, n);
    sortinfo(y, n);
    sortinfo(z, n);
    fr1(i, n - 1)
    {
        int32_t dist = abs(x[i].first - x[i + 1].first);
        edge[++ count] = {x[i].second, x[i + 1].second, dist};
    }
    fr1(i, n - 1)
    {
        int32_t dist = abs(y[i].first - y[i + 1].first);
        edge[++ count] = {y[i].second, y[i + 1].second, dist};
    }
    fr1(i, n - 1)
    {
        int32_t dist = abs(z[i].first - z[i + 1].first);
        edge[++ count] = {z[i].second, z[i + 1].second, dist};
    }
    sort(edge + 1, edge + 1 + count, [&](interstella &a, interstella &b)
    {
        return a.w < b.w;
    });
    int32_t ans = 0;
