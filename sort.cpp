...
struct segment_tree{
    ...
    seggs *st;
    segment_tree(int32_t n)
    {
        st = new seggs[4 * n];
        pos.resize(n);
    }
    int32_t build(int32_t id, int32_t ll, int32_t rr, vector<int32_t> &a)
    {
        if (ll > rr)
        {
            return 0;
        ...
        {
            pos[ll] = id;
            return st[id].val = a[ll];
        }
        st[id].val = min(build(Lcon, a), build(Rcon, a));
        return st[id].val;
    }
    int32_t update(int32_t id, int32_t ll, int32_t rr, int32_t pos, int32_t val)
    {
        if (pos < ll || rr > pos)
        {
            return st[id].val = val;
        }
        return st[id].val = min(update(Lcon, pos, val), update(Rcon, pos, val));
    }
    int32_t query(int32_t id, int32_t ll, int32_t rr, int32_t l, int32_t r)
    {
        if (ll > r || rr < l)
        {
            return INT32_MAX;
        ...
        return min(query(Lcon, l, r), query(Rcon, l, r));
    }
};
/***********************************************************/
...
void solve()
{
    ...
    fr(i, n)
    {
        ...
        int32_t temp = a[b[i]].back();
        a[b[i]].pop_back();
        if (st.query(0, 0, n - 1, 0, temp - 1) < b[i])
        {
            cout << "NO" << endline;
            return;
        }
        st.update(0, 0, n - 1, temp, n + 1);
    }
    cout << "YES" << endline;
    return;
}
