...
void solve(int64_t test_cases)
{
	...
	cin >> r >> R;
	cin >> n;
	priority_queue<pair<long double, int8_t>> a;
	fr1(i, n)
	{
		int32_t x, y;
		cin >> x >> y;
		long double dist = distance(x, y, 0, 0);
		if (dist <= r + R)
			a.push(mp(dist, 1));
	}	
	cin >> m;
	...
	pair<int32_t, int8_t> ans = {0, 0};
	while(not a.empty())
	{
		if (ans.second == 0)
		{
			ans.second = a.top().second;
		}
		if (ans.second != a.top().second)
			break;
		ans.first ++;
		a.pop();
	}	
	if (ans.second == 1)
	{
		cout << ans.first << spaceuk << 0 << endline;
	}
	elif (ans.second == 2)
	{
		cout << 0 << spaceuk << ans.first << endline;
	}
	else
	{
		cout << 0 << spaceuk << 0 << endline;
	}
}
//Functions content:
