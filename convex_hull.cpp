...
vector<descartes> a(maxN), b(maxN);
int32_t n, s1, m;

/***********************************************************/
//Functions declarations:
bool comp(descartes &x, descartes &y)
{
    return (x.x < y.x || (x.x == y.x && x.y < y.y));
}

pair<int32_t, int32_t> Vector(descartes &p, descartes &q)
{
    return mp(q.x - p.x, q.y - p.y);
}
int64_t CCW(descartes &p, descartes &q, descartes &r)
{
    pair<int32_t, int32_t> u = Vector(p, q),
                            v = Vector(q, r);
    return (1ll * u.fi * v.se - 1ll * u.se * v.fi);
}
bool comp2(descartes &p, descartes &q)
{
    int64_t cw = CCW(a[1], p, q);
    if (cw == 0)
        return comp(p, q);
    return cw > 0;
}
void Make_convex(vector<descartes> &a, int32_t n, vector<descartes> &b, int32_t &m)
{
    sort(a.begin() + 1, a.begin() + n + 1, comp);
    sort(a.begin() + 2, a.begin() + n + 1, comp2);
    b[m = 1] = a[1];
    FOR(i, 2, n)
    {
        while(m > 1 && CCW(b[m - 1], b[m], a[i]) <= 0) 
            m --;
        b[ ++m] = a[i];
    }
}
long double KC(descartes &p, descartes &q)
{
    int64_t hx = p.x - q.x;
    int64_t hy = p.y - q.y;
...
void solve()
{
    ...
    b[m + 1] = b[1];
    int64_t area = 0;
    long double peri = 0.0;
    fr1(i, m)
    {
        area += 1ll * (b[i].x - b[i + 1].x) * (b[i].y + b[i + 1].y);
        peri += KC(b[i], b[i + 1]);
    }
    cout << fixed << setprecision(3);
    cout << peri << endline << area / 2 << "." << (area % 2 ? "500" : "000");
    return;
}
//Functions content:
