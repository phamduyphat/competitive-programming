...
void solve()
{
	...
	while(t --)
	{
		int64_t n, maxA = INT64_MIN, minA = INT64_MAX;
		cin >> n;
		vector<int64_t> input(n);
		priority_queue<int64_t> sub;
		forv(v, input)
		{
			cin >> v;
			maxA = max(maxA, v);
		...
		if (minA == input[0] || maxA == input[n - 1])
		{
			cout << maxA - minA << endline;
			continue; 
		}
		input.pb(input[0]);
		fr(i, n)
		{
			sub.push(input[i] - input[i + 1]);
		}
		cout << sub.top() << endline;
	}
}
//Functions content:
