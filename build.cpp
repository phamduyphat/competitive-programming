...
deque<pair<int32_t, int64_t>> input;
vector<int64_t> s1 = {0}, s2 = {0};
/***********************************************************/
...
void solve()
{
    ...
    {
        auto &v = input[i];
        if (v.first == input[id].first)
            input[id].second += v.second;
        else 
            input[++ id] = v;
    }
    fr1(i, id + 1)
    {   
        s1.pb(s1[i - 1] + input[i - 1].second);
        s2.pb(s2[i - 1] + input[i - 1].first * input[i - 1].second);
    }
    input.push_front(mp(0, 0));
    vector<vector<int64_t>> dp(id + 2, vector<int64_t> (2, INT64_MAX));
    dp[0][1] = 0;
    fr1(i, id + 1)
    {
        fr(j, i)
        {
            dp[i][0] = min(dp[i][0], dp[j][1] - (s1[i] - s1[j]) * input[j].first + s2[i] - s2[j]);
        }
