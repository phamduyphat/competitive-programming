...
void solve()
{
    ...
    int64_t n, m;
    cin >> n >> m;
    vector<vector<pair<int64_t, int64_t>>> g(30);
    vector<vector<int64_t>> a(n + 1, vector<int64_t> (m + 1, 0));   
    vector<int64_t> dp((1 << 21), 0);
    fr1(i, m)
    {
        int64_t x, y, z;
        cin >> x >> y >> z;
    ...
    fr(mask, maxN)
    {
        int64_t bit_count = __builtin_popcount(mask);
        int64_t to = maxN ^ mask;
        for(int64_t hi = to; hi > 0; hi ^= hi & (-hi))
        {
            int64_t numhi = __builtin_ctz(hi & (-hi));
            int64_t x = dp[mask] + a[numhi + 1][bit_count + 1];
            forv(v, g[bit_count + 1])
                if (v.fi <= x)
                    x += v.se;
            dp[mask ^ (1 << numhi)] = max(dp[mask ^ (1 << numhi)], x);
        }
    }
    cout << dp[maxN];
    return (void)(0);
