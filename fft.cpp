...
void subsetSums(vector<int32_t> a, int64_t sum, int32_t ll, int32_t rr, int32_t count)
{
    if (count == k) {ans.insert(sum); return;}
    if (ll > rr) return;
    subsetSums(a, sum, ll + 1, rr, count);
    subsetSums(a, sum + a[ll], ll + 1, rr, count + 1);
    subsetSums(a, sum + a[ll], ll, rr, count + 1);
}
/***********************************************************/
driver main(){
    ios_base::sync_with_stdio(0), cin.tie(0), cout.tie(0);
    freopen ("fft.inp","r",stdin);
    freopen ("fft.out","w",stdout);
    //code below this line:
    cin >> n >> k;
    vector<int32_t> a(n);
    forv(v, a) cin >> v;
    subsetSums(a, 0, 0, n - 1, 0);
    forv(v, ans) cout << v << spaceuk;
    //cout << endline;
    //finish();
    return 0;
}
