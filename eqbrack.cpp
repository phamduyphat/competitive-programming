...
/*
    ...
    tạo 1 stack<char>
    gặp kí tự C:
        nếu bắt buộc phải là '(' thì ta đưa vào stack;
        ==> khi nào nó bắt buộc phải là mở ngoặc    
            xét đỉnh stack, nếu là C: pop
            nếu ngc lại: push(c);
    ...
    if (ll > rr) return;
        T[ll] -> S[ll] = '('
        Tìm j lớn nhất <= r: T[j] = T[ll];
            -> S[j] = '('
        solve(ll, j - 1); solve(j + 1, r);    
*/