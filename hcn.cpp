...
driver main(){
    ...
    cin >> m >> n;
	fr1(i, m) fr1(j, n) cin >> a[i][j];
	fr1(x_first, m - 1) FOR(x_second, x_first + 1, m) fr1(y_first, n - 1) FOR(y_second, y_first + 1, n)
	if (a[x_first][y_first] == a[x_second][y_first] && a[x_second][y_first] == a[x_second][y_second] && a[x_second][y_second] == a[x_first][y_second])	ans ++;
	cout << ans << endline;
	//finish();
	return 0;
}
