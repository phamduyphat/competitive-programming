...
//functions content:
/*
    sử dụng một cái 'lưới' để phủ toàn bộ
        chuẩn hoá
        xmin = min(x, x'), ymin = min(y, y')
        xmax = max(x, x'), ymax = max(y, y')

    mảng đánh dấu D[100][100]; | (xmin, ymin) -> (xmax - 1, ymax - 1);
                                => FOR(x, xmin, xmax - 1)
                                    FOR(y, ymin, ymax - 1)  D[x][y] = 1;
