...
void solve()
{
	...
	while(t --)
	{
		...
		int64_t ans = 0;
		cin >> n;
		priority_queue<int32_t> pqa, pqb;
		fr1(i, n)
		{
			int32_t input;
			cin >> input;
		...
		while(not pqa.empty() and not pqb.empty())
		{
			if (pqa.top() == pqb.top())
			{
				pqa.pop();
			...
			elif(pqa.top() > pqb.top())
			{
				pqa.push(digits(pqa.top()));
				pqa.pop();
				ans ++;
			}
			elif(pqa.top() < pqb.top())
			{
				pqb.push(digits(pqb.top()));
				pqb.pop();
				ans ++;
			}
		}
