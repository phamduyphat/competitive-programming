...
int64_t st[4 * (int64_t)(1e5 + 10)];
/***********************************************************/
//functions declarations:
#define MID (ll + (rr - ll) / 2)
#define Lcon id * 2, ll, MID
#define Rcon id * 2 + 1, MID + 1, rr
int64_t update(int32_t id, int32_t ll, int32_t rr, int32_t pos, int32_t val)
{
    if (ll > pos || rr < pos) return 0;
    
    
    st[id] = update(Lcon, pos, val) + update()
